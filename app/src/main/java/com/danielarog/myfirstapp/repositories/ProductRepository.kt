...
object ProductRepository {
    ...
    suspend fun getAllProductsByUserId(
        id: String,
        liveData: MutableLiveData<MutableList<ShoppingItem>>
    ) {
        for (categoryEnum in Category.categoryList()) {
            categoryEnum.category.getSubCategories()
                .forEach { categorySub ->
                    val productCollection =
                        subCategoryCollectionRef(categoryEnum.category, categorySub)
                    // @TODO
                    // query default items here can be done
                    // with conditions on item rating,date etc..
                    val resource = productCollection
                        .whereEqualTo("publisherId", id)
                        .get()
                        .tryAwait()
                    resource?.toObjects(ShoppingItem::class.java).let { list ->
                        list?.let { items ->
                            val newVal = liveData.value
                            newVal?.addAll(items)
